...
def click(x,y):
    """
    Function:
    Moves to mouse to x/y position and simulates a left-click
    :param x: x-value of screenposition (where to move)
    :param y: y-value of screenposition (where to move)
    """
...
def queueGame():
    """
    Function:
    Loops until it finds the "FindMatch" button. Then clicks on it.
    """
    print('Queueing up..')
    clicked = False
    while not clicked:
...
def acceptGame():
    """
    Function:
    Same as queueGame. Accepts the game.
    """
    print('Accepting...')
    clicked = False
    while not clicked:
...
def forfeitGame():
    """
    Function:
    Forfeits the game. Presses ESC button and then it searches for the needed Buttons to ff.
    """
    time.sleep(3)
    print('surrendering...')
...
def sellUnit(x,y):
    """
    Function:
    Moves to mouse to x/y position and simulates a left-click and then sells the unit
    :param x: x-value of screenposition (where to move)
    :param y: y-value of screenposition (where to move)
    """
    time.sleep(0.5)
...
def takeAugment(x,y):
    """
    Function:
    Moves to mouse to x/y position and simulates a left-click to select an Augment
    :param x: x-value of screenposition (where to move)
    :param y: y-value of screenposition (where to move)
    """
    time.sleep(3)
...
def gameProcessing(username):
    """
    Function:
    Checks which stage it is. Based on the stage does following actions:
        - Stage 1-3,2-5,3-5: Sells unit from carousel
        - Stage 2-1, 3-2, 4-2: Reads Augments via text recognition and takes the left one.
          Sends it to a non-local database after.
        - Stage 4-3: Forfeits the game

    :param username: Username that will be send to the database
    """
...
def runAll(username):
    """
    Function:
    Runs all function above in a loop when button in the GUI is pressed.
    :param username: Username that will be send to the database
...
def main():
    """
    Function:
    A GUI, where you can start the program.
    """
    root = tk.Tk()
    apps = []
